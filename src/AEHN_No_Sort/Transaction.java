...
public class Transaction {
    ...
    /** the transaction utility of the transaction or projected transaction */
     float redefinedTransactionUtility; 
      ...
      */
    public Transaction(int[] items, float[] utilities, float redefinedTransactionUtility) {
    	this.items = items;
    	this.utilities = utilities;
    	this.redefinedTransactionUtility = redefinedTransactionUtility;
    	this.offset = 0;
     ...
     */
    public Transaction(Transaction transaction, int offsetE,int offsetNegative) {	
    	...
    	float utilityE = this.utilities[offsetNegative];
    	...
    	this.redefinedTransactionUtility = transaction.redefinedTransactionUtility;
    	...
    	this.offsetNegative = offsetNegative+1;
    }
    public Transaction(Transaction transaction, int offsetE) {	
    	// copy items and utilities from the original transaction
    	this.items = transaction.getItems();
    	this.utilities = transaction.getUtilities();
    	this.offsetNegative = transaction.offsetNegative;
    	// copy the utility of element e
    	...
    	// It is the transaction utility minus the profit of the element that was removed
    	if(utilityE>0) {
    		this.redefinedTransactionUtility = transaction.redefinedTransactionUtility - utilityE;
    	}
    	// and we also need to subtract the utility of all items before e
    	// but after the previous offset
    	for(int i = transaction.offset; i < offsetE; i++){
    		if(transaction.utilities[i]>0) {
    			this.redefinedTransactionUtility -= transaction.utilities[i];
    		}
    	}
    	// remember the offset for this projected transaction
     ...
     */
     public String toString() {
		StringBuilder buffer = new StringBuilder();
		 ...
		 buffer.append(" Remaining Utility:" +redefinedTransactionUtility);
	...
	public void removeUnpromisingItems(int[] oldNamesToNewNames) {
    	...
    	for(int j=0; j< items.length;j++) {
    		...
    		if(newName != 0) {
    			...
    			if (utilities[j]>0) {
    				redefinedTransactionUtility -= utilities[j];
				}
	...
	{
		return redefinedTransactionUtility;
	}
}
