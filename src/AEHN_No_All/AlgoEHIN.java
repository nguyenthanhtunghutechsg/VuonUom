...
public class AlgoEHIN {
	...
	private int[] utilityBinArraySUNegative;
	/**
...
//			System.out.println(dataset.toString());
		...
		for (int j = 1; j < utilityBinArrayLU.length; j++) {
			if (ListNegative.contains(j)||(!ListPositive.contains(j))) {
				//itemsToKeepNegative.add(j);
			}
			if (utilityBinArrayLU[j] >= minUtil) {
				
				if (ListPositive.contains(j)||ListNegativeAndPositive.contains(j)) {
		...
		insertionSort(itemsToKeepNegative, utilityBinArrayLU);
		...
		newItemCount = itemsToKeepNegative.size() + itemsToKeepPositive.size();
		// initialize the utility-bin array for counting the subtree utility
		utilityBinArraySU = new int[newItemCount + 1];
		utilityBinArraySUNegative= new int[newItemCount + 1];
...
//    	//======
	...
	public static void insertionSortReverse(List<Integer> items, int[] utilityBinArrayTWU) {
		// the following lines are simply a modified an insertion sort
...
//		        }
			...
			if (utilityPe > minUtil) {
				backtrackingEFIMNegative(transactionsPe,itemsToKeepNegative,prefixLength+1);
			}
			...
			if (activateSubtreeUtilityPruning) {
				...
				// items
				backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToExplore, itemsToKeepNegative,
						prefixLength + 1);
			} else {
				// if sub-tree utility pruning is deactivated, we consider secondary items also
				// as primary items
				backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToKeep, itemsToKeepNegative, prefixLength + 1);
			}
		}
...
//			}
	...
	private boolean isEqualToNegative(Transaction t1, Transaction t2) {
		// we first compare the transaction lenghts
		int length1 = t1.items.length - t1.offsetNegative;
		int length2 = t2.items.length - t2.offsetNegative;
		// if not same length, then transactions are not identical
		if (length1 != length2) {
			return false;
		}
		// if same length, we need to compare each element position by position,
		// to see if they are the same
		int position1 = t1.offsetNegative;
		int position2 = t2.offsetNegative;
	...
	public void printStats() {
		...
		System.out.println("========== EHIN v97 - STATS "+this.getClass().getName()+ "============");
		System.out.println(" minUtil = " + minUtil);
		System.out.println("Positive: "+ListPositive.size()+" Negative: "+ListNegative.size());
